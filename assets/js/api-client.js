...
/* ==============================================
   API-CLIENT.JS v5.1 - Finnhub API Client
...
class FinanceAPIClient {
    constructor(config = {}) {
        ...
        console.log('🔧 API Client v5.1 initialized with baseURL:', this.baseURL);
    }
    ...
    // ============================================
    // API REQUEST HANDLER (✅ AVEC RATE LIMITING)
    // ============================================
    
    async makeRequest(endpoint, params = {}, retryCount = 0) {
        // ✅ VÉRIFICATION DU QUOTA API (RATE LIMITING)
        if (typeof window.canUseAPI === 'function') {
            const quotaCheck = await window.canUseAPI('finnhub');
            
            if (!quotaCheck.allowed) {
                console.warn('⛔ API quota exceeded:', quotaCheck.reason);
                
                // Afficher la modale d'erreur
                if (typeof window.showQuotaModal === 'function') {
                    window.showQuotaModal('api', quotaCheck);
                }
                
                // Retourner une erreur
                throw new Error(quotaCheck.message || 'API quota exceeded. Please upgrade your plan.');
            }
            
            console.log(`✅ API quota OK. Remaining: ${quotaCheck.remainingDaily}/day, ${quotaCheck.remainingMinute}/min`);
        } else {
    ...
    async getTimeSeries(symbol, interval = '1day', outputsize = 100) {
        try {
            ...
            // Sinon essayer data.values (Twelve Data brut)
                ...
                console.log('⚠ Using raw Twelve Data format (values)');
                timeSeriesData = data.values;
            }
    ...
    async getTechnicalIndicator(symbol, indicator, interval = '1day', timePeriod = 14) {
        console.warn('⚠ Technical indicators endpoint is not available (PREMIUM feature)');
        return null;
    }
    
    async getStatistics(symbol) {
        console.warn('⚠ Statistics endpoint is not available (PREMIUM feature)');
        return null;
    }
}
...
console.log('✅ FinanceAPIClient v5.1 loaded - WITH RATE LIMITING!');