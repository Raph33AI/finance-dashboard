...
// ============================================
// CHATBOT FULL PAGE UI - VERSION ULTRA v2.0
// Key Metrics + Comparaison + Charts
// ============================================
...
class ChatbotFullPageUI {
    ...
    async init() {
        try {
            ...
            console.log('✅ Full Page UI v2.0 initialized');
            console.log('📊 NEW: Key Metrics Cards + Comparison Charts');
    ...
    // ============================================
    // ✅ SEND MESSAGE (MODIFIÉ POUR KEY METRICS)
    // ============================================
    async sendMessage(messageText = null) {
        if (!this.elements.input) {
            console.error('❌ Input element not found!');
        ...
        try {
            ...
            // ✅ NOUVEAU : Afficher Key Metrics AVANT les graphiques
            if (response.keyMetrics && this.charts) {
                await this.displayKeyMetrics(response.keyMetrics);
            }
            
            // ✅ Générer graphiques (avec support comparaison)
            if (response.chartRequests && response.chartRequests.length > 0) {
                await this.generateCharts(response.chartRequests);
            }
    ...
    // ============================================
    // ✅ NOUVELLE MÉTHODE : DISPLAY KEY METRICS
    // ============================================
    async displayKeyMetrics(keyMetrics) {
        if (!this.elements.messages || !this.charts) return;
        
        console.log('📊 Displaying Key Metrics...');
        
        const metricsContainer = document.createElement('div');
        metricsContainer.className = 'metrics-container-wrapper';
        
        // Si c'est un array (comparaison), afficher plusieurs cartes
        if (Array.isArray(keyMetrics)) {
            keyMetrics.forEach(stockData => {
                this.charts.createKeyMetricsCard(stockData, metricsContainer);
            });
        } 
        // Si c'est un objet unique (stock simple)
        else {
            this.charts.createKeyMetricsCard(keyMetrics, metricsContainer);
        }
        
        this.elements.messages.appendChild(metricsContainer);
        this.scrollToBottom();
    ...
    // ============================================
    // ✅ GENERATE CHARTS (MODIFIÉ POUR COMPARAISON)
    // ============================================
    async generateCharts(chartRequests) {
        if (!this.elements.messages || !this.charts) return;
        ...
        for (const request of chartRequests) {
            ...
            // ✅ NOUVEAU : Support pour les graphiques de comparaison
            if (request.type === 'comparison' && request.symbols && request.symbols.length >= 2) {
                console.log(`📊 Creating comparison chart: ${request.symbols.join(' vs ')}`);
                await this.charts.createComparisonChart(request.symbols, chartContainer, request.timeframe || '1y');
            } 
    ...
    clearMessages() {
        if (this.elements.messages) {
            const messages = this.elements.messages.querySelectorAll('.message, .chart-message, .metrics-container-wrapper, #typing-indicator');
            messages.forEach(msg => msg.remove());
        }
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    try {
        window.financialChatbotFullPage = new ChatbotFullPageUI(ChatbotConfig);
        console.log('✅ Chatbot Full Page v2.0 ready!');
    } catch (error) {
        console.error('❌ Initialization error:', error);
    }
});