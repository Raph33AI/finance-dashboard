...
/**
 * ════════════════════════════════════════════════════════════════
 * MARKET SENTIMENT DASHBOARD - Avec Tracking Entreprises/Pays
 * ════════════════════════════════════════════════════════════════
 */
...
class MarketSentimentDashboard {
    ...
    createArticleCard(article) {
        ...
        if (article.companies && article.companies.length > 0) {
            entitiesBadges += article.companies.slice(0, 3).map(company => `
                <span class='entity-badge company-badge' title='${company.ticker}'>
                    <i class='fas fa-building'></i> ${company.name}
                </span>
            `).join('');
        }
        if (article.countries && article.countries.length > 0) {
            entitiesBadges += article.countries.slice(0, 2).map(country => `
                <span class='entity-badge country-badge'>
        ...
        card.innerHTML = `
            ...
            <div class='article-title-small'>${article.title}</div>
            ${entitiesBadges ? `<div class='article-entities'>${entitiesBadges}</div>` : ''}
            <div class='article-meta'>
                <span class='article-source-small'>
                    <i class='fas fa-rss'></i> ${article.sourceName}
                </span>
