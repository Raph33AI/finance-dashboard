...
/**
 * ═══════════════════════════════════════════════════════════════════
 * 📊 INSIDER FLOW TRACKER - ULTRA-POWERFUL PREMIUM VERSION V5
 * ═══════════════════════════════════════════════════════════════════
 * AlphaVault AI - Advanced Insider Trading Analysis Dashboard
 * 
 * ✅ CORRECTIONS V5 :
 * - Panneau de configuration avant chargement (comme IPO Intelligence)
 * - Sélection ticker + période + nombre de filings
...
(function() {
    ...
    function hidePanelAndShowDashboard() {
        const panel = document.getElementById('dataLoaderPanel');
        const dashboard = document.getElementById('dashboardContent');
    ...
    function updateEstimatedTime() {
        const countSelect = document.getElementById('configFilingsSelect');
        ...
        if (count <= 50) estimate = '~2 seconds';
        else if (count <= 100) estimate = '~3-5 seconds';
        else if (count <= 200) estimate = '~5-8 seconds';
        else estimate = '~8-12 seconds';
        
        timeEstimate.textContent = estimate;
    }

    async function startAnalysis() {
        const ticker = document.getElementById('configTickerInput').value.trim().toUpperCase();
        const timePeriod = parseFloat(document.getElementById('configPeriodSelect').value);
        const filingCount = parseInt(document.getElementById('configFilingsSelect').value);
        
        if (!ticker) {
            showNotification('⚠ Please enter a ticker symbol', 'warning');
            document.getElementById('configTickerInput').focus();
            return;
        }

        console.log(`🎯 User selected: ${ticker}, ${timePeriod} months, ${filingCount} filings`);
        
        userConfig.ticker = ticker;
        userConfig.timePeriod = timePeriod;
        userConfig.filingCount = filingCount;
        
        hidePanelAndShowDashboard();
        
        await analyzeInsiderActivity(ticker, timePeriod, filingCount);
    }
    ...
    function updateDatasetInfoDisplay() {
        ...
        const ticker = userConfig.ticker;
        const txCount = currentAnalysis.transactions.length;
        const period = userConfig.timePeriod;
        
        infoElement.innerHTML = `
            <i class="fas fa-database"></i> 
            <strong>${ticker}</strong> • 
            <strong>${txCount} transactions</strong> loaded 
            | <i class="fas fa-calendar-alt"></i> 
            Period: <strong>${period} months</strong>
            | <i class="fas fa-file-alt"></i>
            <strong>Form 4 (Insider Trading)</strong>
        `;
    }
                    ...
                    ">
                        ...
                        ">
                            <div style="
                                ...
                                margin: 0 auto 20px;
                            "></div>
                            <h3 style="color: white; margin: 0 0 10px; font-size: 1.5rem;">Analyzing Insider Activity</h3>
                            <p id="loadingMessage" style="color: rgba(255, 255, 255, 0.9); margin: 0; font-size: 1rem;">
                                Fetching Form 4 filings from SEC EDGAR...<br>
                                <small>(Analyzing ${userConfig.ticker})</small>
                            </p>
                        </div>
                    </div>
                    <style>
    ...
    // ═══════════════════════════════════════════════════════════════
    async function analyzeInsiderActivity(ticker, months, maxFilings) {
        if (typeof InsiderAnalyticsEngine === 'undefined') {
            console.error('❌ InsiderAnalyticsEngine is not defined!');
            showNotification('❌ Error: Analytics engine not loaded. Check console.', 'error');
            return;
        ...
        try {
            currentAnalysis = await analyticsEngine.analyzeCompany(ticker, {
                ...
                onProgress: (progress, message) => {
                    ...
                    if (loadingMsg) {
                        loadingMsg.innerHTML = `${message}<br><small>(Analyzing ${ticker})</small>`;
                    }
                }
            });
            ...
            if (resultsContainer) {
                resultsContainer.innerHTML = `
                    <div class="error-state">
                        ...
                        <h2>Analysis Failed</h2>
                        <p style="color: var(--text-secondary); margin-top: 12px;">${error.message}</p>
                        <button class="primary-btn" onclick="window.InsiderFlowTracker.showConfigPanel()" style="margin-top: 24px;">
    ...
    function initializeEventListeners() {
        // ✅ Config Panel Events
        document.getElementById('configAnalyzeBtn')?.addEventListener('click', startAnalysis);
        
        document.getElementById('configTickerInput')?.addEventListener('keypress', (e) => {
            if (e.key === 'Enter') {
                startAnalysis();
            }
        });
        
        document.getElementById('configFilingsSelect')?.addEventListener('change', () => {
            updateEstimatedTime();
        });
        ...
        document.getElementById('refreshData')?.addEventListener('click', () => {
            if (userConfig.ticker) {
                analyzeInsiderActivity(userConfig.ticker, userConfig.timePeriod, userConfig.filingCount);
            } else {
                showNotification('⚠ Please configure analysis first', 'warning');
                showConfigPanel();
            }
        });
    ...
    console.log('🎉 Insider Flow Tracker - PREMIUM VERSION V5 initialized successfully!');
