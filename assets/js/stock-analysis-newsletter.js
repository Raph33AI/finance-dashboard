/**
 * ════════════════════════════════════════════════════════════════
 * STOCK ANALYSIS NEWSLETTER SYSTEM V2.1 - ULTRA-PROFESSIONAL
 * ✅ Ajout des logos d'entreprises (Multi-Fallback System)
 * ✅ Logo normalisé en pièce jointe (optimisé anti-pixélisation)
 * ✅ Conservation du style original des analyses
...
class StockAnalysisNewsletter {
    ...
    getLogoUrls(ticker, companyName) {
        ...
        return {
            // ✅ TAILLE NORMALISÉE : 256x256 pour éviter la pixélisation
            primary: `https://img.logo.dev/${domain}?token=pk_X-WazSBJQn2GwW2hy9Lwpg&size=256`,
            fallbacks: [
    ...
    async initialize() {
        console.log('Initializing Stock Analysis Newsletter System V2.1...');
    ...
    async generateStockAnalysis(symbol, forceManual = false) {
        try {
            ...
            // ✅ LOGO EN PIÈCE JOINTE (taille normalisée 256x256)
            const postImages = [];
            if (logoUrl) {
    ...
    generatePremiumContent(symbol, companyName, logoUrl) {
        ...
        // ✅ HEADER PREMIUM AVEC LOGO D'ENTREPRISE
        ...
        if (logoUrl) {
            md += `<div style="width: 120px; height: 120px; margin: 0 auto 20px; background: white; border-radius: 20px; padding: 12px; box-shadow: 0 8px 24px rgba(0, 0, 0, 0.2); display: flex; align-items: center; justify-content: center;">\n\n`;
        ...
        md += `<p style="font-size: clamp(1rem, 2.5vw, 1.3rem); font-weight: 700; margin: 0 0 16px 0; color: #ffffff;">${companyName}</p>\n\n`;
...
console.log('Stock Analysis Newsletter System V2.1 - Script Loaded (Reuses Advanced Analysis Methods + Company Logos)');