...
// Comparaison + Analyse Historique + Key Metrics
// ============================================
...
class FinancialChatbotEngine {
    constructor(config) {
        ...
        this.conversationContext = {
            lastSymbol: null,
            lastSymbols: [], // ✅ NOUVEAU : pour comparaisons
            lastTimeframe: '1y',
            lastTopic: null,
            userPreferences: {}
        };
    ...
    async initialize() {
        try {
            ...
            console.log('🚀 Financial AI v5.0 ULTRA ready!');
            console.log('📊 NEW: Stock Comparison + Historical Analysis + Key Metrics');
    ...
    async processMessage(userMessage) {
        ...
        try {
            ...
            // ✅ NOUVEAU : Générer chartRequests et keyMetrics automatiquement
            response.chartRequests = this.generateChartRequests(analysis, context);
            response.keyMetrics = this.generateKeyMetrics(analysis, context);
            response.suggestions = this.generateSmartSuggestions(analysis, context);
            response.processingTime = performance.now() - startTime;
            ...
            console.log(`✅ Response generated in ${response.processingTime.toFixed(0)}ms`);
            console.log(`📊 Chart requests: ${response.chartRequests.length}`);
            console.log(`🔑 Key metrics: ${response.keyMetrics ? 'YES' : 'NO'}`);
    ...
    // ============================================
    // ✅ ANALYSE COMPLÈTE DU MESSAGE (AMÉLIORÉE)
    // ============================================
    analyzeMessage(message) {
        const lowerMessage = message.toLowerCase();
        ...
        // ✅ NOUVEAU : Détection COMPARISON
        if (this.detectComparisonIntent(message, symbols)) {
            type = 'STOCK_COMPARISON';
            intents.push('COMPARISON');
        }
        
        // ✅ NOUVEAU : Détection HISTORICAL_ANALYSIS
        if (this.detectHistoricalIntent(message, timeframes)) {
            type = symbols.length > 0 ? 'HISTORICAL_ANALYSIS' : type;
            intents.push('HISTORICAL');
        }
        
        // Détections existantes (ordre de priorité)
        if (this.detectIPOIntent(message)) {
            type = type === 'CONVERSATIONAL' ? 'IPO_QUERY' : type;
            intents.push('IPO');
        }
    ...
    detectComparisonIntent(message, symbols) {
        const messageLower = message.toLowerCase();
        
        // Patterns de comparaison
        const comparisonPatterns = [
            /\b(compare|comparison|versus|vs\.?|against)\b/i,
            /\b(difference between|better than|worse than)\b/i,
            /\b(which is better|which one)\b/i,
            /\b(how does .+ stack up)\b/i
        ];
        
        // Doit avoir au moins 2 symboles OU pattern de comparaison
        const hasComparisonKeyword = comparisonPatterns.some(pattern => pattern.test(messageLower));
        const hasMultipleSymbols = symbols.length >= 2;
        
        // Pattern "AAPL with MSFT" ou "AAPL and GOOGL"
        const hasWithAnd = /\b(with|and)\b/i.test(messageLower) && symbols.length >= 2;
        
        return (hasComparisonKeyword && symbols.length >= 1) || hasMultipleSymbols || hasWithAnd;
    }
    
    detectHistoricalIntent(message, timeframes) {
        const keywords = [
            'performance', 'evolution', 'historical', 'history', 'trend',
            'over the last', 'in the last', 'past', 'since',
            'how did', 'how has', 'show me', 'chart', 'graph'
        ];
        const messageLower = message.toLowerCase();
        
        // Doit avoir un keyword historique + timeframe (ou number + year/month/etc)
        const hasHistoricalKeyword = keywords.some(keyword => messageLower.includes(keyword));
        const hasTimeframe = timeframes.length > 0 || /\b\d+\s*(year|month|week|day)s?\b/i.test(message);
    ...
    // ============================================
    // CONSTRUCTION DE CONTEXTE (IDENTIQUE)
    // ============================================
    ...
    async buildSmartContext(message, analysis) {
        ...
        // ✅ SI COMPARAISON : Charger données pour TOUS les symboles
        if (analysis.type === 'STOCK_COMPARISON' && analysis.symbols.length >= 2) {
            console.log(`\n📊 ═══ LOADING COMPARISON DATA FOR ${analysis.symbols.join(' vs ')} ═══`);
            
            context.comparisonData = [];
            
            for (const sym of analysis.symbols) {
                try {
                    const stockData = await this.analytics.getStockData(sym).catch(() => null);
                    if (stockData) {
                        context.comparisonData.push(stockData);
                        console.log(`   ✅ ${sym}: $${stockData.quote.current} (${stockData.quote.changePercent}%)`);
                    }
                } catch (error) {
                    console.warn(`   ⚠ Error loading ${sym}:`, error);
                }
            }
        ...
        // ✅ Charger données pour le symbole principal (comme avant)
        if (symbol) {
            console.log(`\n📊 ═══ LOADING COMPREHENSIVE DATA FOR ${symbol} ═══`);
    ...
    // ============================================
    // ✅ GÉNÉRATION AUTOMATIQUE DE CHART REQUESTS
    // ============================================
    
    generateChartRequests(analysis, context) {
        const requests = [];
        
        // ✅ COMPARAISON : Créer un chart de comparaison
        if (analysis.type === 'STOCK_COMPARISON' && analysis.symbols.length >= 2) {
            const timeframe = analysis.timeframes[0] || '1y';
            requests.push({
                type: 'comparison',
                symbols: analysis.symbols.slice(0, 3), // Max 3 symboles
                timeframe: timeframe,
                data: timeframe
            });
            
            console.log(`   📊 Added comparison chart: ${analysis.symbols.join(' vs ')}`);
        }
        
        // ✅ HISTORICAL : Créer un chart historique
        else if (analysis.type === 'HISTORICAL_ANALYSIS' && analysis.symbols.length > 0) {
            const symbol = analysis.symbols[0];
            const timeframe = analysis.timeframes[0] || '5y';
            
            requests.push({
                type: 'line',
                symbol: symbol,
                data: timeframe,
                indicators: ['sma'] // Ajouter SMA 20/50
            });
            
            console.log(`   📊 Added historical chart: ${symbol} (${timeframe})`);
        }
        
        // ✅ STOCK QUERY classique : Chart simple
        else if (analysis.type === 'STOCK_QUERY' && analysis.symbols.length > 0 && !requests.length) {
            const symbol = analysis.symbols[0];
            const timeframe = analysis.timeframes[0] || '1y';
            
            // Détecter si besoin de candlestick ou line
            const needsCandlestick = /\b(candlestick|ohlc|candle)\b/i.test(context.userMessage);
            
            requests.push({
                type: needsCandlestick ? 'candlestick' : 'line',
                symbol: symbol,
                data: timeframe,
                indicators: /\b(sma|moving average|ma)\b/i.test(context.userMessage) ? ['sma'] : []
            });
            
            console.log(`   📊 Added stock chart: ${symbol} (${timeframe})`);
        }
    ...
    // ============================================
    // ✅ GÉNÉRATION AUTOMATIQUE DE KEY METRICS
    // ============================================
    
    generateKeyMetrics(analysis, context) {
        // Générer Key Metrics si :
        // - Stock query simple
        // - Comparaison (pour chaque symbole)
        // - Historical analysis
        
        if (context.stockData && (analysis.type === 'STOCK_QUERY' || analysis.type === 'HISTORICAL_ANALYSIS')) {
            return context.stockData;
        }
        
        if (context.comparisonData && context.comparisonData.length > 0) {
            return context.comparisonData; // Array de stockData
        }
    ...
    // ============================================
    // PROMPT ENRICHI (IDENTIQUE - trop long à répéter)
    // ============================================
    ...
    buildEnhancedPrompt(message, context) {
        ...
        // ✅ SI COMPARAISON
        if (context.comparisonData && context.comparisonData.length >= 2) {
            prompt += `\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
            prompt += `📊 **STOCK COMPARISON**\n`;
            prompt += `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n`;
            
            context.comparisonData.forEach((stock, index) => {
                prompt += `**${index + 1}. ${stock.profile?.name || stock.symbol}** (${stock.symbol})\n`;
                prompt += `• Price: $${stock.quote.current} (${stock.quote.changePercent > 0 ? '+' : ''}${stock.quote.changePercent}%)\n`;
                prompt += `• Market Cap: $${stock.profile?.marketCap || 'N/A'}B\n`;
                prompt += `• P/E Ratio: ${stock.metrics?.peRatio || 'N/A'}\n`;
                prompt += `• EPS: $${stock.metrics?.eps || 'N/A'}\n`;
                prompt += `• Beta: ${stock.metrics?.beta || 'N/A'}\n`;
                prompt += `• Dividend Yield: ${stock.metrics?.dividendYield || '0'}%\n`;
                prompt += `• 52-Week Range: $${stock.metrics?.week52Low || 'N/A'} - $${stock.metrics?.week52High || 'N/A'}\n\n`;
            });
            
            prompt += `**Your Task:** Provide a detailed comparison of these stocks, highlighting key differences, strengths, and weaknesses. Be specific with numbers.\n\n`;
        }

        // Reste du prompt (identique à avant - données stock, analyst, earnings, etc.)
        if (context.stockData) {
            const stock = context.stockData;
            prompt += `\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
            prompt += `📊 **${stock.profile?.name || stock.symbol}** (${stock.symbol})\n`;
        ...
        // ... (reste du prompt identique - analyst recommendations, price target, earnings, etc.)
        // Pour gagner de la place, je ne répète pas tout le code identique
    ...
    // ============================================
    // CALCULS TECHNIQUES (IDENTIQUE - je ne répète pas)
    // ============================================
    ...
    generateSmartSuggestions(analysis, context) {
        ...
        // ✅ SI COMPARAISON : Suggestions de comparaison
        if (analysis.type === 'STOCK_COMPARISON' && analysis.symbols.length >= 2) {
            const symbols = analysis.symbols;
            suggestions.push(
                `Which is better: ${symbols[0]} or ${symbols[1]}?`,
                `${symbols[0]} vs ${symbols[1]} technical analysis`,
                `Show me ${symbols[0]} and ${symbols[1]} earnings comparison`,
                `${symbols[0]} vs ${symbols[1]} price targets`
            );
        }
        
        // ✅ SI HISTORICAL : Suggestions temporelles
        else if (analysis.type === 'HISTORICAL_ANALYSIS' && analysis.symbols.length > 0) {
            const symbol = analysis.symbols[0];
            suggestions.push(
                `${symbol} performance over 10 years`,
                `${symbol} volatility analysis`,
                `${symbol} drawdown history`,
                `${symbol} vs market performance`
            );
        }
        
        // ✅ STOCK QUERY classique
        else if (analysis.symbols.length > 0) {
            const symbol = analysis.symbols[0];
            ...
            suggestions.push(
                ...
                "📊 Show TSLA performance over 5 years",
    ...
    getCompanySymbolMapping() {
        return {
            ...
            'coinbase': 'COIN', 'paypal': 'PYPL', 'visa': 'V',
            'mastercard': 'MA', 'jpmorgan': 'JPM', 'disney': 'DIS',
    ...
    updateConversationContext(analysis) {
        if (analysis.symbols.length > 0) {
            this.conversationContext.lastSymbol = analysis.symbols[0];
            this.conversationContext.lastSymbols = analysis.symbols; // ✅ NOUVEAU
        }
