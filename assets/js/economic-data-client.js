 ...
 * ALPHAVAULT AI - ECONOMIC DATA API CLIENT
 * ====================================================================
 * Client JavaScript pour le Cloudflare Worker
 * Version: 5.0 - FRED (US) + ECB (Forex)
 * 
 * Worker URL: https://economic-data-worker.raphnardone.workers.dev
 * 
 * Supporte:
 * - 34 endpoints FRED (US)
 * - 2 endpoints ECB (Forex EUR)
 * - Cache intelligent
 * - Gestion d'erreurs
 * - Formatage de données pour Chart.js
 */
...
class EconomicDataClient {
    ...
    /**
     * ========================================
     * ✅ ECB FOREX API METHODS (NOUVEAUX)
     * ========================================
     */

    /**
     * Récupère TOUS les taux de change EUR (30+ devises)
     */
    async getECBAllExchangeRates() {
        const response = await this.call('/ecb/rates', {}, { cacheTTL: 1800000 }); // 30 min
        return {
            success: response.success,
            rates: response.rates || {},
            date: response.date,
            source: response.source || 'European Central Bank',
            timestamp: response.timestamp
        };
    }

    /**
     * Récupère l'historique EUR vs une devise
     * @param {string} currency - Code devise (USD, GBP, JPY, etc.)
     * @param {string} startDate - Format YYYY-MM-DD
     * @param {string} endDate - Format YYYY-MM-DD
     */
    async getECBHistoricalExchangeRate(currency, startDate, endDate) {
        const response = await this.call('/ecb/historical', {
            currency: currency,
    ...
    /**
     * Extrait les observations ECB en format standardisé
     * (Pour compatibilité avec l'ancien code)
     */
    extractECBObservations(data) {
...
// Test de connexion au chargement
if (typeof console !== 'undefined') {
    console.log('📊 Economic Data Client loaded v5.0');
    console.log('🔗 Worker URL:', window.economicDataClient.baseUrl);
    console.log('🇺🇸 FRED Series:', Object.keys(FRED_SERIES).length);
    console.log('🇪🇺 ECB Forex: 30+ currencies');
}
