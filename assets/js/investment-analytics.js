...
(function() {
    ...
    const InvestmentAnalytics = {
        ...
        // ✅ ═══════════════════════════════════════════════════════════════
        // 🏢 COMPANY LOGOS SYSTEM (Multi-API avec Fallback)
        // ═══════════════════════════════════════════════════════════════
        companyDomains: {
            'AAPL': 'apple.com', 'MSFT': 'microsoft.com', 'GOOGL': 'abc.xyz',
            'AMZN': 'amazon.com', 'META': 'meta.com', 'TSLA': 'tesla.com',
            'NVDA': 'nvidia.com', 'NFLX': 'netflix.com', 'ADBE': 'adobe.com',
            'CRM': 'salesforce.com', 'ORCL': 'oracle.com', 'INTC': 'intel.com',
            'AMD': 'amd.com', 'QCOM': 'qualcomm.com', 'AVGO': 'broadcom.com',
            'TXN': 'ti.com', 'CSCO': 'cisco.com', 'IBM': 'ibm.com',
            'ACN': 'accenture.com', 'NOW': 'servicenow.com', 'INTU': 'intuit.com',
            'PYPL': 'paypal.com', 'SQ': 'squareup.com', 'SNOW': 'snowflake.com',
            'PLTR': 'palantir.com', 'ZM': 'zoom.us', 'SHOP': 'shopify.com',
            'UBER': 'uber.com', 'LYFT': 'lyft.com', 'ABNB': 'airbnb.com',
            'DASH': 'doordash.com', 'SNAP': 'snap.com', 'PINS': 'pinterest.com',
            'SPOT': 'spotify.com', 'RBLX': 'roblox.com', 'U': 'unity.com',
            'JPM': 'jpmorganchase.com', 'BAC': 'bankofamerica.com',
            'WFC': 'wellsfargo.com', 'C': 'citigroup.com', 'GS': 'goldmansachs.com',
            'MS': 'morganstanley.com', 'V': 'visa.com', 'MA': 'mastercard.com',
            'COIN': 'coinbase.com', 'JNJ': 'jnj.com', 'UNH': 'unitedhealthgroup.com',
            'PFE': 'pfizer.com', 'LLY': 'lilly.com', 'ABBV': 'abbvie.com',
            'MRK': 'merck.com', 'ABT': 'abbott.com', 'TMO': 'thermofisher.com',
            'WMT': 'walmart.com', 'COST': 'costco.com', 'HD': 'homedepot.com',
            'TGT': 'target.com', 'NKE': 'nike.com', 'KO': 'coca-colacompany.com',
            'PEP': 'pepsico.com', 'MCD': 'mcdonalds.com', 'SBUX': 'starbucks.com',
            'XOM': 'exxonmobil.com', 'CVX': 'chevron.com', 'BA': 'boeing.com',
            'CAT': 'caterpillar.com', 'HON': 'honeywell.com', 'GE': 'ge.com',
            'SAP': 'sap.com', 'ASML': 'asml.com', 'LVMUY': 'lvmh.com',
            'NVO': 'novonordisk.com', 'SHEL': 'shell.com', 'BP': 'bp.com',
            'BABA': 'alibaba.com', 'TCEHY': 'tencent.com', 'TSM': 'tsmc.com',
            'SONY': 'sony.com', 'TM': 'toyota.com', 'SPY': 'spdrs.com',
            'QQQ': 'invesco.com', 'VOO': 'vanguard.com', 'IVV': 'ishares.com',
            'AGG': 'ishares.com', 'BND': 'vanguard.com', 'LQD': 'ishares.com',
            'BTC-USD': 'bitcoin.org', 'ETH-USD': 'ethereum.org'
        },

        autoGenerateDomain: function(ticker, companyName) {
            if (!companyName && !ticker) return 'example.com';
            
            const name = (companyName || ticker).toLowerCase()
                .replace(/\s+inc\.?$/i, '')
            ...
            const exceptions = {
                ...
                's&p500': 'spdrs.com',
                'sp500': 'spdrs.com',
                'nasdaq100': 'invesco.com',
                'nasdaq': 'invesco.com',
                'bonds': 'ishares.com',
                'bondetf': 'vanguard.com'
            };
            
            return exceptions[name] || `${name}.com`;
        },

        getCompanyLogoUrl: function(ticker, companyName) {
        ...
        handleAssetLogoError: function(img, fallbacksData, initials) {
            try {
                const fallbacks = JSON.parse(fallbacksData);
                
                if (fallbacks.length > 0) {
                    const nextUrl = fallbacks.shift();
                    img.parentElement.dataset.fallbacks = JSON.stringify(fallbacks);
                    img.src = nextUrl;
                } else {
                    // Dernier recours : afficher les initiales
                    const logoDiv = img.parentElement;
                    logoDiv.innerHTML = `<div class='asset-logo-initials'>${initials}</div>`;
                }
            } catch (e) {
                console.error('Error handling logo fallback:', e);
                img.parentElement.innerHTML = `<div class='asset-logo-initials'>${initials}</div>`;
            }
        },
        ...
        renderAssetsList: function() {
            ...
            container.innerHTML = assets.map(asset => {
                ...
                // ✅ LOGO SYSTEM: Générer les URLs et initiales
                const logoData = self.getCompanyLogoUrl(asset.ticker, asset.name);
                const initials = (asset.name || asset.ticker || 'N/A')
                    .split(' ')
                    .map(word => word[0])
                    .join('')
                    .toUpperCase()
                ...
                const fallbacksJson = JSON.stringify(logoData.fallbacks).replace(/'/g, '&apos;');
                
                return `
                    <div class='asset-item ${asset.type}'>
                        <div class='asset-info'>
                            <!-- ✅ LOGO avec fallback multi-API -->
                            <div class='asset-logo-container' data-fallbacks='${fallbacksJson}'>
                                <img src='${logoData.primary}' 
                                    alt='${self.escapeHtml(asset.name)}' 
                                    class='asset-company-logo'
                                    onerror='InvestmentAnalytics.handleAssetLogoError(this, \`${fallbacksJson}\`, "${initials}")'>
                            </div>
                            
                            <div class='asset-details'>
                                <div class='asset-name'>${self.escapeHtml(asset.name)}</div>
                                <div class='asset-ticker'>
                                    ${asset.ticker ? `<span class='ticker-badge'>${asset.ticker}</span>` : '<span class="no-ticker">No ticker</span>'}
                                    <span class='asset-type-badge ${asset.type}'>${self.formatAssetType(asset.type)}</span>
                                </div>
                            </div>
                        </div>
                        <div class='asset-allocation-display'>
                            <div class='allocation-bar'>
                                <div class='allocation-fill' style='width: ${Math.min(asset.allocation, 100)}%; background: ${self.assetColors[asset.type]}'></div>
                            </div>
                            <div class='allocation-percent'>${allocationDisplay}</div>
                        </div>
                        <div class='asset-actions'>
                            <button class='asset-btn edit' data-asset-id='${asset.id}' data-action='edit' title='Edit asset'>
                                <i class='fas fa-edit'></i>
                            </button>
                            <button class='asset-btn delete' data-asset-id='${asset.id}' data-action='delete' title='Delete asset'>
